    ...
    <div className="">
      ...
      <div className="grid grid-cols-2 gap-10 container mx-auto p-10">
        {services.map((service, index) => (
          <article key={index} className=" relative overflow-hidden rounded-lg shadow transition hover:shadow-lg group hover:cursor-pointer">
            <Image
              alt=""
              src={service.image}
              width={764}
            ...
            <div className="relative bg-gradient-to-t from-gray-900/70 to-gray-900/25 pt-32 sm:pt-48 lg:pt-64">
              <div className="p-4 sm:p-6">
                <Link href={service.link || "#"} target="_blank">
                  <h3 className="mt-0.5 text-lg text-white">{t(service.title)}</h3>
                </Link>

                <p className="mt-2 line-clamp-3 text-sm/relaxed text-white/95">{t(service.content || "")}</p>
              </div>
            </div>
          </article>
        ))}
