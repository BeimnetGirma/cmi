...
const MAGAZINE_UPLOAD_DIR = "public/uploads/magazine";

export const responseLimit = "50mb";

// Routes to upload pdf files and save them to the database.
export async function POST(req: NextRequest) {
  const uploadDir = path.join(process.cwd(), MAGAZINE_UPLOAD_DIR);
  ...
  const filePath = path.join(process.cwd(), MAGAZINE_UPLOAD_DIR, fileName);
...
export async function GET(req: NextRequest) {
  ...
  // const filePath = path.join(process.cwd(), MAGAZINE_UPLOAD_DIR, filename);
  const baseDir = path.resolve(process.cwd(), MAGAZINE_UPLOAD_DIR);
