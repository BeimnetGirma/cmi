...
const AnnouncementMenu: React.FC<PageProps & { announcements: announcement[] }> = ({ announcements, params: { lng } }) => {
  ...
  const hasNewAnnouncements = (announcements: announcement[]) => {
    ...
    return announcements.some((announcement: announcement) => {
      ...
      console.log("Announcement Time ****", announcementTime);
  ...
  return (
    <li
      className={`py-4 px-2 hover:bg-slate-200 hover:text-primary-main ${
        pathname === `/${lng}/announcements` ? "font-semibold text-primary-main bg-slate-200 " : "font-normal text-secondary-light"
      }`}
    >
